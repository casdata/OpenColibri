...
/*
float thermA =  0.6989527046e-3;              //Calculated Stainhart-Hart model coefficients
float thermB =  2.948656661e-4;
float thermC =  -1.381899722e-7;
...
float thermA =  0.9430468202e-3;              //Calculated Stainhart-Hart model coefficients
float thermB =  2.480482099e-4;
float thermC =  1.009595903e-7;
...
int getBit2Word(const uint8_t *word, int8_t position){
...
void readTemp(uint16_t *thermTemp){
    ...
    if(readMCP3421_ADC(&adcValue)){
        ...
        //ESP_LOGW(TEMP_TASK_TAG, "t= %.2f", temp);
...
void setLcdEntryMode(const bool incrementMode, const bool shift){
...
void setLcdOptions(const bool displayOn, const bool cursorOn, const bool cursorBlink) {
...
void setLcdCursorOrShift(const bool displayShift, const bool rightShift){
...
void setLcdFunction(const bool bitDL, const bool bitN, const bool bitF){
...
void setLcdCGRAM_addr(uint8_t addr){
 ...
 */
void setLcdDDRAM_addr(uint8_t addr){
...
void write2LCD(const char *dataStr, const uint8_t dataStrLength){
...
void setLcdPos(const uint8_t x){
    setLcdDDRAM_addr(x);
}
...
static void mainTask(void* pvParameters){
    ...
    write2LCD("Open Colibri V1.00 2023", 23);
    ...
    */
    ...
    /*
    sendData2LCD(0b00100001, 0, 1);             //Write 'W'

    vTaskDelay(pdMS_TO_TICKS(200));

    sendData2LCD(0b10110001, 0, 1);             //Write 'W'

    vTaskDelay(pdMS_TO_TICKS(200));

    sendData2LCD(0b11111111, 0, 1);             //Write 'W'

    vTaskDelay(pdMS_TO_TICKS(200));

    sendData2LCD(0b01111110, 0, 1);             //Write 'W'
    */
